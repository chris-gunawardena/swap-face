...
(async () => {
  // ─────────────────────────────────────────────
  // 1) Grab OpenCV.js, Haar cascade, and ONNX Runtime UMD bundle
  // ─────────────────────────────────────────────
  const [opencvText, cascadeBuf, ortText] = await Promise.all([
    fetch(chrome.runtime.getURL('opencv/opencv.full.js')).then(r => r.text()),
    fetch(chrome.runtime.getURL('opencv/haarcascade_frontalface_default.xml')).then(r => r.arrayBuffer()),
...
`;
  ...
  // ─────────────────────────────────────────────
  // 3) Build & spawn the swap worker with WebGL→Wasm fallback
  // ─────────────────────────────────────────────
  const modelUrl = chrome.runtime.getURL('models/simswap_quant.onnx');
  const swapWorkerSrc = `
${ortText}
...
(async () => {
  const url = ${JSON.stringify(modelUrl)};
  for (const backend of ['webgl','wasm']) {
    try {
      session = await ort.InferenceSession.create(url, { executionProviders: [backend] });
      postMessage({ type: 'swap-ready', backend });
      return;
    } catch (e) {
      // try next backend
    }
  }
  postMessage({ type: 'swap-error', error: 'no available backend found' });
})();

onmessage = async ({ data }) => {
  if (data.type !== 'swap') return;
  if (!session) {
    postMessage({ type: 'swap-error', error: 'session not initialized' });
    return;
  }
  ...
  try {
    ...
    const outData = outputMap.out.data;
...
`;
  ...
  swapWorker.onmessage = ({ data }) => {
    if (data.type === 'swap-ready') {
      console.log('🔄 Swap worker ready (backend):', data.backend);
    } else if (data.type === 'swap-error') {
      console.error('Swap worker error:', data.error);
    } else if (data.type === 'swap-done') {
      ...
      const faceImg = new ImageData(new Uint8ClampedArray(data.pixels), 256, 256);
